...
images = 1; %2 to have the program generate .bmp files of the stimuli otherwise 1
...
trialnr = 0;
        for NumberOrder = 1:balance;% 1 = small-large / 2 = large-small
            for S1Trials = 1:length(S1);
                trialnr = trialnr+1;
                CondMatrix(1,trialnr) = NumberOrder;
                CondMatrix(2,trialnr) = S1(1,S1Trials);
                CondMatrix(3,trialnr) = S2(1,S1Trials);
                if CondMatrix(1,trialnr) == 1;
                    if S1(1,S1Trials)>S2(1,S1Trials); CondMatrix(5,trialnr)=1; CondMatrix(4,trialnr) = S2(1,S1Trials)/S1(1,S1Trials);
                    else  CondMatrix(5,trialnr) = 2; CondMatrix(4,trialnr) = S1(1,S1Trials)/(S2(1,S1Trials));
                    end
                else
                    if S1(1,S1Trials)>S2(1,S1Trials); CondMatrix(5,trialnr)=2;CondMatrix(4,trialnr) = S2(1,S1Trials)/S1(1,S1Trials);
                    else  CondMatrix(5,trialnr) = 1;CondMatrix(4,trialnr) = S1(1,S1Trials)/(S2(1,S1Trials));
                    end
                end
            end
        end
...
for block = 1:nrofblocks;
    ...
    for j = 1:length(CondMatrix);
        ...
        W = CondMatrix(4,TrialListCond(1,j));
        if CondMatrix(1,TrialListCond(1,j)) == 1 && design ~=4;
            F = CondMatrix(2,TrialListCond(1,j));
            S = CondMatrix(3,TrialListCond(1,j));
        else
            F = CondMatrix(3,TrialListCond(1,j));
            S = CondMatrix(2,TrialListCond(1,j));
        end



        if  CondMatrix(5,TrialListCond(1,j)) == 1;
            if SZ == 1;
                matrixff = matrix3;
                matrixll = matrix4;
            else
